...
const JourneyCard = ({ title, description, status, duration, nextAction, onAction, actionLabel = 'Continue', className }) => {
    ...
    return (<Card className={cn('transition-all duration-200 hover:shadow-lg', getStatusStyles(), className)}>
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center space-x-3">
          {getStatusIcon()}
          <h3 className="text-lg font-semibold text-white">{title}</h3>
        </div>
        {duration && (<span className="text-sm text-zinc-400">{duration}</span>)}
      </div>

      <p className="text-zinc-300 mb-4">{description}</p>

      {nextAction && (<div className="mb-4 p-3 bg-zinc-800/50 rounded-lg">
          <p className="text-sm text-zinc-400">
            <span className="font-medium">Next:</span> {nextAction}
          </p>
        </div>)}

      {onAction && (<button onClick={onAction} className={cn('w-full flex items-center justify-center space-x-2 px-4 py-2 rounded-lg font-medium transition-colors', status === 'completed'
