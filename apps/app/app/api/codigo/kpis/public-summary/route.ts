...
function readProvidedPassword(req: NextRequest): string {
  const header = (req.headers.get('x-codigo-kpi-password') || '').trim();
  if (header) return header;
  const { searchParams } = new URL(req.url);
  return (searchParams.get('password') || '').trim();
}

export async function GET(req: NextRequest) {
  try {
    const required = (process.env.CODIGO_KPI_PASSWORD || '').trim();
    if (!required) {
      return NextResponse.json(
        { error: 'Missing required env: CODIGO_KPI_PASSWORD' },
        { status: 501 }
      );
    }
    ...
    const premiumAttachmentRate =
      premiumSet.size > 0 && sessionsCount > 0 ? premiumSessions / sessionsCount : null;
    ...
    const idleAvgSecs = gapsSecs.length
      ? Math.round(gapsSecs.reduce((a, b) => a + b, 0) / gapsSecs.length)
      : null;
    ...
    console.error('[CODIGO KPIs Public] Error:', error);
    return NextResponse.json(
