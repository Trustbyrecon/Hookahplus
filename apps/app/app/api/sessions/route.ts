...
export const POST = withRequestContext(async (req: NextRequest): Promise<NextResponse> => {
  ...
  try {
    ...
    // Canonical QR path: resolve table -> session + participant deterministically.
    // This prevents duplicate active sessions for the same table.
    const sourceHint = String(body.source || '').toUpperCase();

    // Enforce: QR source MUST provide loungeId + tableId and MUST go through resolver.
    if (sourceHint === 'QR' && (!body?.loungeId || !body?.tableId)) {
      console.error('[Sessions API] Rejecting QR create without loungeId/tableId');
      return NextResponse.json(
        {
          success: false,
          error: 'MISSING_QR_CONTEXT',
          message: 'QR session creation requires loungeId and tableId.',
          next: 'USE_CANONICAL_RESOLVER',
        },
        { status: 400, headers: getCorsHeaders(req) }
      );
    }

    const hasCanonicalResolverInputs =
      !!body.tableId &&
      !!body.loungeId &&
      sourceHint === 'QR';
    ...
    if (hasCanonicalResolverInputs) {
      ...
      const displayName = String(body.customerName || 'Guest').trim() || 'Guest';
      const notMe = Boolean(body.notMe);
      ...
      const resolved = await resolveSessionParticipant(prisma as any, {
        loungeId,
        tableId,
        identityKey,
        displayName,
      ...
      if (resolved.mode === 'blocked_multi_active') {
        return NextResponse.json(
          {
            success: false,
            blocked: true,
            ...
            conflictSessionIds: resolved.conflictSessionIds || [],
          },
      ...
      if (!session) {
        return NextResponse.json(
          { success: false, error: 'Resolved session not found' },
      ...
      return NextResponse.json(
        {
          ...
          message:
            resolved.mode === 'create'
              ...
              : resolved.mode === 'rejoin'
                ? 'Rejoined active session'
