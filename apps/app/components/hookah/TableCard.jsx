...
const TableCard = ({ table, onAction, className }) => {
    ...
    return (<Card className={cn('hover:shadow-lg transition-all duration-200', className)}>
      ...
      {/* Action Buttons */}
      <div className="flex items-center space-x-2">
        {table.status === 'available' && (<Button size="sm" variant="primary" onClick={() => onAction?.('fire_session', table.id)}>
            Fire Session
          </Button>)}
        
        {table.status === 'occupied' && (<Button size="sm" variant="warning" onClick={() => onAction?.('hold_session', table.id)}>
            Hold Session
          </Button>)}
        
        {table.status === 'reserved' && (<Button size="sm" variant="info" onClick={() => onAction?.('check_in', table.id)}>
            Check In
          </Button>)}
        
        {table.status === 'cleaning' && (<Button size="sm" variant="success" onClick={() => onAction?.('mark_ready', table.id)}>
            Mark Ready
          </Button>)}
        
        <Button size="sm" variant="outline" onClick={() => onAction?.('view_details', table.id)}>
          View Details
        </Button>
      </div>
    </Card>);
