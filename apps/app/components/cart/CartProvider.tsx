...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("hp_app_cart");
  ...
  useEffect(() => {
    try {
      localStorage.setItem("hp_app_cart", JSON.stringify(items));
    } catch {}
  }, [items]);
  ...
  const value = useMemo(() => ({ items, add, remove, clear, subtotal, itemCount }), [items, subtotal, itemCount]);
