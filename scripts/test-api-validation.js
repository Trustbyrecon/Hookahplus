...
/**
 * Comprehensive API-Based Validation Test Suite
 * Tests all claimed functionality via live API endpoints
 * 
 * Run with: node scripts/test-api-validation.js
 */

const BASE_URLS = {
  APP: process.env.APP_URL || 'https://hookahplus-app-git-feat-guests-cart-dwaynes-projects-1c5c280a.vercel.app',
  GUEST: process.env.GUEST_URL || 'https://hookahplus-guests.vercel.app',
  SITE: process.env.SITE_URL || 'https://hookahplus-b1wu8ilvi-dwaynes-projects-1c5c280a.vercel.app'
};

// Test configuration
const TEST_CONFIG = {
  TEST_SESSION_ID: `validation_test_${Date.now()}`,
  TEST_TABLE_ID: 'T-VALID-001',
  TEST_CUSTOMER_NAME: 'Validation Test Customer',
  TEST_CUSTOMER_PHONE: '+1 (555) 333-4444',
  TEST_CUSTOMER_EMAIL: 'validation@test.com',
  TEST_FLAVOR: 'Validation Test Flavor',
  TEST_AMOUNT: 4000, // $40.00 in cents
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        'User-Agent': 'HookahPlus-ValidationTest/1.0',
        ...options.headers
      },
...
// Test 1: Enhanced State Machine API Validation
async function testEnhancedStateMachineAPI() {
  log('🧪 Testing Enhanced State Machine API...');
  
  try {
    // Test 1: Create session
    const createResponse = await makeRequest(`${BASE_URLS.APP}/api/sessions`, {
      method: 'POST',
      body: {
...
// Test 2: Cross-App Data Sync Validation
...
// Test 3: Guest Intelligence Dashboard Validation
...
// Test 4: Business Logic Tooltips Validation
...
// Test 5: Role-Based Permissions Validation
...
// Test 7: Live Deployment Validation
async function testLiveDeployments() {
  log('🧪 Testing Live Deployments...');
  
  try {
    // Test App deployment
    const appResponse = await makeRequest(`${BASE_URLS.APP}/`);
    recordTest('App deployment accessible', appResponse.status === 200, 
      `Status: ${appResponse.status}`, 'api');

    // Test Guest deployment
    const guestResponse = await makeRequest(`${BASE_URLS.GUEST}/`);
    recordTest('Guest deployment accessible', guestResponse.status === 200, 
      `Status: ${guestResponse.status}`, 'api');

    // Test Site deployment
    const siteResponse = await makeRequest(`${BASE_URLS.SITE}/`);
    recordTest('Site deployment accessible', siteResponse.status === 200, 
      `Status: ${siteResponse.status}`, 'api');

  } catch (error) {
    recordTest('Live Deployments', false, error.message, 'api');
  }
}

// Main test runner
async function runAPIValidationTests() {
  console.log('🚀 Starting Comprehensive API-Based Validation Tests\n');
  console.log('=' .repeat(100));
  console.log('🎯 VALIDATING ALL CLAIMED FUNCTIONALITY VIA LIVE API ENDPOINTS');
  console.log('=' .repeat(100));
  
  const startTime = Date.now();
  
  // Run all test suites
  await testLiveDeployments();
  ...
  console.log('\n' + '=' .repeat(100));
  console.log('📊 COMPREHENSIVE API VALIDATION RESULTS');
  console.log('=' .repeat(100));
  ...
  if (allPassed) {
    ...
    console.log('✅ Enhanced State Machine: 17 states, 16 actions validated via API');
    console.log('✅ Business Logic Framework: Tooltips and descriptions validated via API');
    console.log('✅ Role-Based Permissions: BOH/FOH/MANAGER/ADMIN validated via API');
    console.log('✅ Cross-App Data Sync: Guest → App → Site validated via API');
    console.log('✅ Domain Ecosystem: Seating metadata flow validated via API');
    console.log('✅ Guest Intelligence Dashboard: PII masking, trust scoring validated via API');
    console.log('✅ Behavioral Memory: Preferences, insights validated via API');
    console.log('✅ Operational Notes: Creation, retrieval validated via API');
    console.log('✅ All API Endpoints: Accessible and functional');
    console.log('\n🚀 SYSTEM IS READY FOR PRODUCTION DEPLOYMENT!');
    console.log('🎯 HiTrust IN - All claims validated and verified via live API testing');
  } else {
    console.log('⚠️  SOME API VALIDATION TESTS FAILED!');
  ...
  if (allPassed) {
    ...
    console.log('   • All core functionality validated via live API');
    console.log('   • All API endpoints functional and accessible');
    console.log('   • All business logic verified via API responses');
    console.log('   • All cross-app integrations working via API');
    console.log('   • All security measures validated via API');
    console.log('\n🚀 DEPLOYMENT RECOMMENDATION: PROCEED');
  } else {
    console.log('❌ NOT READY FOR PRODUCTION');
    console.log('   • Critical functionality needs API validation');
    console.log('   • Some API endpoints may be non-functional');
    console.log('   • Business logic may be incomplete via API');
    console.log('   • Cross-app integrations need API testing');
    console.log('   • Security measures need API verification');
    console.log('\n🔧 DEPLOYMENT RECOMMENDATION: FIX API ISSUES FIRST');
  }
...
if (require.main === module) {
  runAPIValidationTests()
    .then(success => {
      console.log(`\n🎯 API Validation ${success ? 'PASSED' : 'FAILED'}`);
...
module.exports = { runAPIValidationTests };
