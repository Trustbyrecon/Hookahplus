...
const SessionCard = ({ session, onAction, className }) => {
    ...
    return (<Card className={cn('hover:shadow-lg transition-all duration-200', className)}>
      ...
      <div className="space-y-3 mb-4">
        ...
        {/* Coal Status */}
        {session.coalStatus && (<div className="flex items-center justify-between">
      ...
      {/* Action Buttons */}
      <div className="flex items-center space-x-2">
        <Button size="sm" variant="success" leftIcon={<RefreshCw className="w-4 h-4"/>} onClick={() => onAction?.('restart_prep', session.id)}>
          Restart Prep
        </Button>
        
        <Button size="sm" variant="info" leftIcon={<UserCheck className="w-4 h-4"/>} onClick={() => onAction?.('resolve_issue', session.id)}>
          Resolve Issue
        </Button>
        
        <Button size="sm" variant="danger" leftIcon={<Flag className="w-4 h-4"/>} onClick={() => onAction?.('flag_manager', session.id)}>
          Flag Manager
        </Button>
        
        <Button size="sm" variant="warning" leftIcon={<Pause className="w-4 h-4"/>} onClick={() => onAction?.('hold_session', session.id)}>
          Hold Session
        </Button>
        
        <Button size="sm" variant="accent" leftIcon={<Zap className="w-4 h-4"/>} onClick={() => onAction?.('request_refill', session.id)}>
          Request Refill
        </Button>
      </div>
    </Card>);
