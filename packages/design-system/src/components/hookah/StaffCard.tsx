...
export interface StaffCardProps {
  staff: {
    id: string;
    name: string;
    role: 'manager' | 'foh' | 'boh' | 'host' | 'admin';
...
const StaffCard: React.FC<StaffCardProps> = ({ staff, onAction, className }) => {
        ...
        <div className="flex items-center space-x-3">
          <div className="w-12 h-12 bg-zinc-700 rounded-full flex items-center justify-center">
            {staff.avatar ? (
              ...
              <span className="text-2xl">{getRoleIcon(staff.role)}</span>
            )}
          </div>
          <div>
        ...
        {staff.assignedTables && staff.assignedTables.length > 0 && (
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-1">
              {staff.assignedTables.map((tableId, index) => (
      ...
      <div className="flex items-center space-x-2">
        {staff.status === 'active' && (
          <Button
            size="sm"
            variant="warning"
            onClick={() => onAction?.('send_break', staff.id)}
          >
            Send Break
          </Button>
        )}
        ...
        {staff.status === 'break' && (
          <Button
            size="sm"
            variant="success"
            onClick={() => onAction?.('end_break', staff.id)}
          >
            End Break
          </Button>
        )}
        ...
        {staff.status === 'off' && (
          <Button
            size="sm"
            variant="primary"
            onClick={() => onAction?.('start_shift', staff.id)}
          >
            Start Shift
          </Button>
        )}
        ...
        <Button
          size="sm"
          variant="outline"
          onClick={() => onAction?.('view_profile', staff.id)}
        >
          View Profile
        </Button>
        ...
        <Button
          size="sm"
          variant="outline"
          onClick={() => onAction?.('assign_tables', staff.id)}
        >
          Assign Tables
        </Button>
      </div>
